...
import com.business.utils.CompareUtils;
...
public class CronField {
    ...
    public List<Integer> points() {
        ...
        if (containsComma()) {
            ...
            if (listCache.size() > 1) {
                //去重
                CompareUtils.removeDuplicate(listCache);
                //排序
        ...
        if (containsHyphen()) {
            ...
            CompareUtils.assertRange(cronPosition, left);
            //1-32/2的情况
            if (strings[1].contains(SLASH)) {
                String[] split = strings[1].split(SLASH);
                //32
                right = Integer.parseInt(split[0]);
                CompareUtils.assertSize(left, right);
                CompareUtils.assertRange(cronPosition, right);
                //2
            ...
            CompareUtils.assertRange(cronPosition, single);
