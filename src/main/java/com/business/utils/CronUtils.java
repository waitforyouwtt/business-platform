...
import com.business.entity.CronField;
import com.business.entity.CronPosition;
import com.business.entity.DayOfYear;
import com.business.entity.TimeOfDay;
...
public class CronUtils {
    ...
    private static Date doDayOfYear(int addYear, Calendar calendar, CronField fieldDay, CronField fieldMonth, CronField fieldWeek, CronField fieldYear, TimeOfDay timeOfDayMin) {
        ...
        int year = DateUtils.year(calendar);
        //有年域的情况
        if (null != fieldYear) {
        ...
        //先确定日月
        int dayNow = DateUtils.day(calendar);
        int monthNow = DateUtils.month(calendar);
    ...
    private static TimeOfDay doTimeOfDay(Calendar calendar, CronField fieldSecond, CronField fieldMinute, CronField fieldHour) {
        //先确定时分秒
        int hourNow = DateUtils.hour(calendar);
        int minuteNow = DateUtils.minute(calendar);
        int secondNow = DateUtils.second(calendar);
        ...
        if (timeOfDayNow.compareTo(timeOfDayMin) < 0) {
            ...
            TimeOfDay next = CompareUtils.findNext(timeOfDayNow, points);
    ...
    public static List<TimeOfDay> timesOfDay(String cron, Date date) {
        ...
        int year = DateUtils.year(calendar);
        int week = DateUtils.week(calendar);
        int month = DateUtils.month(calendar);
        int day = DateUtils.day(calendar);
        /// 如果包含年域
        if (CRON_LEN_YEAR == cronFields.size()) {
    ...
    public static boolean satisfy(int fieldValue, CronField field) {
        ...
        return field.containsAll() || CompareUtils.inList(fieldValue, field.points());
    }
    ...
    public static List<CronField> convertCronField(String cron) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            cronFields.add(new CronField(
                    cronPosition,
                    CronShapingUtils.shaping(cut.get(i), cronPosition)
            ));
        }
        return cronFields;
    }
